...
export default function Profile() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (profilePhotoFile) {
        const uploadResult = await UploadFile({ file: profilePhotoFile });
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="flex items-center gap-6">
                 <img src={previewUrl || `https://ui-avatars.com/api/?name=${formData.full_name.replace(' ', '+')}&background=random`} alt="Profile" className="w-24 h-24 rounded-full object-cover border-4 border-white shadow-lg"/>
